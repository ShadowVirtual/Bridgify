...
const { isGuildPremium, isUserPremium } = require("../functions/isGuildPremium.js");
module.exports = {
    data: new SlashCommandBuilder()
        .setName("partner-category")
        .setDescription("[PREMIUM] Partner with a random guild under a specific category")
        .addStringOption(option => (
            option.setName("category")
            .setDescription("The category of the server you want to partner with")
            .setRequired(true)
            .addChoices(
    ...
    async execute(interaction) {
        ...
        const [randomServerWithAd, randomAd, yourGuildData] = await Promise.all([
            guildData.aggregate([
                ...
                { $match: { "category": interaction.options.getString("category") } },
                { $match: { 
                    $or: [
        ...
        if (!(await isGuildPremium(interaction.guildId) || await isUserPremium(interaction.user.id, interaction.client, interaction.guildId))) {
            await interaction.editReply({embeds: [await errorCodes(3, interaction.guildId)]});
            return
        }
        ...
        collector.on('collect', async interaction => {
            if (interaction.customId === "Check_Ad") {
                ...
                await interaction.reply({ embeds: [adEmbed], ephemeral: true });
            }
            if (interaction.customId === 'acceptButton') {
                ...
                const acceptButton2 = new ButtonBuilder()
                    ...
                    .setStyle(ButtonStyle.Success)
                    .setCustomId(`accept-${randomStringCode}-${interaction.guild.id}-${randomServerWithAd.guildId}-partner_category`)
                    .setEmoji(emojis.checkmark)
                const checkAd = new ButtonBuilder()
                    .setLabel(await translator("Check Ad", "English", randomServerWithAd.language || "English"))
                    .setStyle(ButtonStyle.Secondary)
                    .setCustomId(`check_ad-${randomStringCode}-${interaction.guild.id}-${randomServerWithAd.guildId}-partner_category`)
                    .setEmoji(emojis.glass)
                const denyButton2 = new ButtonBuilder()
                    .setLabel(await translator("Decline", "English", randomServerWithAd.language || "English"))
                    .setStyle(ButtonStyle.Danger)
                    .setCustomId(`deny-${randomStringCode}-${interaction.guild.id}-${randomServerWithAd.guildId}-partner_category`)
                    .setEmoji(emojis.cross)
                const blacklistButton = new ButtonBuilder()
                    .setLabel(await translator("Blacklist", "English", randomServerWithAd.language || "English"))
                    .setStyle(ButtonStyle.Danger)
                    .setCustomId(`blacklist-${randomStringCode}-${interaction.guild.id}-${randomServerWithAd.guildId}-partner_category`)
                ...
                await interaction.update({ content: await translator("I've declined the request! If you want to partner with a different server please use `/partner-category` again in 2 minutes", "English", language || "English"), components: [], embeds: [] });
            }
        });
    }
};
